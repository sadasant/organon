...
> For work governed by the Organon, every capitalized ontological term retains the meaning defined here. A quarantined term carries no binding ontological meaning.
...
**Causal path** is a sequence of Transformations sharing one Direction in which the output State of each Transformation supplies part of the input State of the next. Its arrow is inherited from the Direction of its Transformations; reversing the sequence does not preserve the same Causal path.
...
### 16. Claim, witness, and evidence

**Claim** is a Representation asserted by an Agent about a State, Relation, or Change.

**Witness** is an Entity distinct from the Agent making a Claim whose relevant Observation is produced through a Causal path containing at least one Constraint outside that Agent's Boundary.

**Evidence** is an Observation admitted by a Rule whose Scope includes that Observation and a Claim on which it bears.

**Attestation** is a Claim made by a Witness about an Observation or Evidence under a Specification of the Witness's identity and Scope.

Evidence can support or defeat a Claim; it does not become truth by definition. No Agent's Claim independently certifies itself.

### 17. Principal, permission, authority, and role

**Order** is a persistent Configuration of Constraints, Records, Interfaces, and recurring Relations through which a plurality of Agents coordinate Action.
...
**Declaration** is a Claim made by an Agent with Authority, carrying a Specification, and recorded by an Order as fixing the Scope in which subsequent Actions, Claims, Rules, or Evidence will count.
...
Stable Flows can make recurring solutions valuable; Centers can absorb those solutions; an Organ can preserve them beyond their originator. These are possible transformations among defined terms, not additional kinds of Agency.
...
**Person** is an Entity to which an Order grants Standing to serve as a Principal or bear Consequences.
...
### 19. Political order and constituent action
...
1. **Dependency closure:** A binding definition may use only the metalanguage, Absence and its axioms, or terms defined earlier in the dependency order.

2. **One term, one meaning:** A term cannot change meaning between technical, political, and editorial contexts without an explicit new term or projection.

3. **No projection inflation:** Different descriptions of one Entity do not create different Entities or kinds of Agency.

4. **No Absence collapse:** Missingness, zero, omission, silence, shadow, gap, and exception are Presences inside fields. None is Absence.

5. **No map-Reality collapse:** A Perception, Model, Map, Canonical system, or Ledger never equals Reality.
...
9. **No metaphor promotion:** A metaphor may reveal a Relation, but resemblance alone cannot establish an Entity, causal claim, or Invariant.
