...
**Truth** is the Relation among a Claim, the Specification declared as its truth condition, and the Presence in Reality within the Claim's Scope to which that Specification applies. Truth obtains exactly when the relevant Presence conforms to the Specification.
...
| `isTrue` | Claim, declared truth-condition Specification, relevant Presence in Reality, Scope | correspondence obtains independently of access, Evidence, proof, consensus, or Admission |
| `trusts` | trusting Entity, other Entity, future contribution, Causal path, Exposure or Consequence, Scope | accepted dependency without determining Control; no entailment of confidence, Permission, or favorable outcome |
| `alignsUnder` | subject Configuration, target Presence, Specification, Scope | directional conformity under this profile only; no identity, Truth, Authority, or cross-profile entailment |
...
14. <!-- organon:claim C14 --> **No truth-status collapse:** Truth neither entails nor is entailed by Agent access, Evidence, proof, consensus, Admission, or institutional standing. A Claim and its target Presence do not entail Truth unless the declared truth-condition Specification is satisfied.

15. <!-- organon:claim C15 --> **No trust-confidence-control collapse:** Trust requires scoped exposure or consequential dependence on another Entity's future contribution without determining Control. Confidence, prediction, history, Evidence, Permission, Authority, incentive compatibility, or favorable Consequences neither separately constitutes Trust nor follows from it.
