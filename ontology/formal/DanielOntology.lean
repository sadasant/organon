...
This file is a noncanonical formalization experiment corresponding to the
binding Markdown ontology. It encodes selected anti-collapse obligations and
institutional dependencies; it does not claim term-for-term parity.

An uninhabited type is a local formal shadow of absolute Absence inside Lean's
already-present metatheory, not absolute Absence itself. Successful elaboration
is external Evidence about this artifact, not an object-level theorem about the
compiler or Reality.
-/
