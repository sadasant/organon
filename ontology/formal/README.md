...
- An Agent-level Intelligence witness constructs different Models and Interpretations for two non-enumerated States and produces conforming Consequences, while a fixed Interpretation cannot satisfy the adaptive relation. Operative Knowledge requires a capable interpreter and discriminating Record; the finite model separates it from Truth and dormant storage. Knowledge Transmission reconstructs a conforming function in a recipient while allowing different Records and Models, and copied data alone supplies no recipient knowledge.
