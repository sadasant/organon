...
- A finite World shadow carries nonempty participants and States, distinct access paths, a scoped Causal path, and a common Invariant. Its model excludes one present State and lets access paths disagree without losing the shared Invariant.
- A finite Substrate shadow carries persistent source States, nonempty Constraints, a scoped Causal path, and explicit Feeds witnesses. Its model separates persistent inputs from invariant-preserving outputs; carrier-to-content realization remains an explicit open gate.
...
The formal spike does not yet cover the complete ontology, generate the Markdown projection, prove satisfiability beyond its small models, settle identity through time beyond a Boundary-indexed Invariant, connect Operationalization to the full Map and Evidence structures, define general consequence semantics, encode a complete Environment-Sense-Perception access path, define Map-to-World fidelity or a universal carrier-realization relation, or define a universal consciousness condition. Local countermodel predicates prove only the stated anti-entailments without pretending to formalize those complete regions.
