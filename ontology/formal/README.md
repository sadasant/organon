...
- Flow carries two distinct Transformation occurrences, a typed `FlowRule` that owns the exact executable similarity Specification, and a `PersistenceWitness` whose ordered State history is exactly the occurrence-output list. A private Ritual supplies an exact participant-history access path for every Flow occurrence and a `RitualUptake` for every later occurrence. The uptake retains the prior contribution endpoint as Memory, contains the later Interpretation occurrence in the sustaining path, and constructively proves that changing Memory changes the Interpretation result. `TargetContinuity` permits unequal target States while requiring one ordered Persistence history and Invariant; every Flow occurrence and both Meaning-contribution origins belong to that history. The shadow does not yet type those local State roles as canonical Perception, Memory, or Interpretation, or construct represented-target Denotation.
