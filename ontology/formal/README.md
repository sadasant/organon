...
- Flow carries two distinct Transformation occurrences, a typed `FlowRule` that owns the exact executable similarity Specification, and a `PersistenceWitness` whose ordered State history is exactly the occurrence-output list. A private Ritual supplies an exact participant-history access path for every Flow occurrence and requires one typed target component in every occurrence output. Meaning shares the Ritual's carrier, Direction, and feeding relation; both paths in its Causal Contribution begin with exact Flow occurrences, so the target at both causal origins is derived rather than asserted through an unrelated Scope, and the selected endpoint equals the participant State. The shadow does not yet prove that the endpoint is a canonical Perception, Interpretation, or Action.
