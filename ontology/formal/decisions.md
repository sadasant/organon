...
`RitualAccess` is indexed by an exact Flow-list position, begins with the Transformation at that position, and ends in the participant Entity's ordered Persistence history. Ritual carries one exact `FlowClassification`. `RitualUptake` carries distinct prior and current indices with a strict `priorIndex < currentIndex` proof, proves both indexed occurrences conform and stand in the Flow's recurrence Relation, then makes the retained prior endpoint load-bearing: a constructive two-input Interpretation function maps current Perception and Memory to the later State, while substituting a contrastive Memory provably changes that result; the sustaining Causal Contribution begins with the compared Flow occurrences and contains the later Interpretation occurrence. Ritual quantifies access over every index and uptake over every noninitial index, so repeated equal Transformation values cannot discharge multiple positions with one witness. The current core does not expose canonical Sense, Perception, Memory, or Interpretation structures with which to type those State roles. The proposal and release preserve that as an open parity gate rather than promoting proposal-local labels by assertion.

Lean is deliberately stricter than binding RM-D2 at one seam: `perceptionChangesInterpretation` also requires a contrastive Perception to change Interpretation, while the canonical prose requires only contrastively load-bearing Memory. This extra field is not a binding requirement; a purely anticipatory Interpretation that is insensitive to the current Perception can satisfy the prose while failing the present Lean shadow.

`TargetContinuity` replaces exact target equality with an ordered history whose States all satisfy one Invariant. Ritual projects the target component of every qualifying Flow output into that exact history. `Meaning` is the participant-target Relation, not a State classifier or a value stored in the target. It names one actual Ritual uptake and joins its current Causal Contribution either by equality to that uptake's contribution or by a left- or right-path start at the uptake's interpreted State; the selected endpoint must equal the participant's current State. The constituting uptake's two target States inhabit the same target history and satisfy its Invariant. Meaning's Scope and Relation use an explicit stable participant-identity index rather than the participant's current State. The finite model proves nonidentity both across its transmitted case and across two participant indices occupying the same current State. An earlier draft accepted an arbitrary target through Scope membership, then overcorrected to exact target equality; both were rejected. Another draft allowed an unrelated sustaining contribution and represented participant identity by current State; both admitted counterexamples to the binding account. Represented-target continuity remains gated until exact Denotation joins are constructed.
