...
`SovereigntySemantics` exposes the predicates used by four structures instead of assigning one Boolean sovereign flag. Constituent Sovereignty requires persistent constituent power and an actual constituting exercise. Constituted Sovereignty requires Standing and Authority across an inhabited Action Scope plus absence of a recognized superior under the declared comparison. Boundary Sovereignty requires scoped controlled admitted and blocked crossings plus an Order-indexed changed enforcement outcome. External Sovereignty requires Recognition by a distinct Order and a witnessed Action in which the target participates as its own Principal. One combined finite world proves compatibility, while four profile-only worlds each inhabit exactly one structure and make the other three uninhabitable. Those are the pairwise non-entailment countermodels. The shadow does not derive its own-Principal predicate from the core `Principal` and `ActsFor` machinery or prove that its other toy predicates arise from complete Organon Orders, Authority, Control, or Enforcement; those joins remain parity gates.
